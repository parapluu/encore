    ...
    where
      ...
      path = do
        ...
        where
          ...
          pathComponent = do
            emeta <- meta <$> getPosition
            optAccess <- option [] $ try question
            access <- (try (dot >> (compartmentAccess <|> varOrCall)))
                        <|> (withLinebreaks bang >> methodCall)
            if null optAccess
            then return access
            else return $ Option emeta access
