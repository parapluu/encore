...
typ  =  try arrow
  ...
  where
      nonArrow =  fut
              ...
              <|> parens nonArrow
      arrow = do
          lhs <- parens (commaSep typ) <|> do {ty <- nonArrow ; return [ty]}
          reservedOp "->"
          rhs <- nonArrow
...
classDecl = do
    pos <- getPosition
    refKind <- option activeRefTypeWithParams $ do
        reserved "passive"
        return passiveRefTypeWithParams
    reserved "class"
    cname <- identifier
    params <- option [] $ do
        ids <- angles $ commaSep identifier
...
expression = buildExpressionParser opTable expr
  where
      opTable = [ [prefix "not" Identifiers.NOT]
      ...
      typedExpression = Postfix $ do
          pos <- getPosition
          reservedOp ":"
          t <- typ
          return (\e -> TypedExpr (meta pos) e t)
      messageSend = Postfix $ do
          pos <- getPosition
          bang
          name <- identifier
