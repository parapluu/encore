...
module Parser.OldParser(parseEncoreProgram) where
...
expr  =  embed
    ...
    where
      ...
      closure = do pos <- getPosition
                   ...
                   reservedOp "->"
                   body <- expression
                   return $ Closure (meta pos) params Nothing body
      ...
      foreach = do pos <- getPosition
                   ...
                   item <- identifier
                   reserved "in"
                   fail "Encore no longer supports foreach"
      ...
      peer = do pos <- getPosition
                reserved "peer"
                ty <- typ
                fail "Encore no longer supports peer"
