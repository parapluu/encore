...
-- that @putChildren (getChildren e) e == e@ and
-- @getChildren (putChildren l e) == l@
...
putChildren (body : es) e@(Let{decls}) =
    e{body = body, decls = zipWith (\(name, _) e -> (name, e)) decls es}
putChildren eseq e@(Seq {}) = e{eseq = eseq}
putChildren [cond, thn, els] e@(IfThenElse {}) =
    e{cond = cond, thn = thn, els = els}
...
putChildren [future, chain] e@(FutureChain {}) =
    e{future = future, chain = chain}
...
foldrAll f e (Program _ _ funs classes) =
    map (foldFunction f e) funs : map (foldClass f e) classes
  where
...
extendAccumProgram :: (acc -> Expr -> (acc, Expr)) -> acc -> Program
                   -> (acc, Program)
extendAccumProgram f acc0 (Program etl imps funs classes) =
    (acc2, Program etl imps funs' classes')
  where
      ...
      extendAccumClass f acc cls@(Class{methods}) =
          (acc', cls{methods = methods'})
        where
...
-- | @filter cond e@ returns a list of all sub expressions @e'@ of @e@ for
-- which @cond e'@ returns @True@
...
extractTypes (Program _ _ funs classes) =
    List.nub $ concat $ concatMap extractFunctionTypes funs ++
      concatMap extractClassTypes classes
  where
      extractFunctionTypes Function {funtype, funparams, funbody} =
          typeComponents funtype : map extractParamTypes funparams ++
            [extractExprTypes funbody]
      extractClassTypes Class {cname, fields, methods} = [cname] :
          map extractFieldTypes fields ++ concatMap extractMethodTypes methods
      extractFieldTypes Field {ftype} = typeComponents ftype
      extractMethodTypes m = typeComponents (mtype m) :
          map extractParamTypes (mparams m) ++ [extractExprTypes (mbody m)]
      extractParamTypes Param {ptype} = typeComponents ptype
      extractExprTypes = foldr (\e acc ->
                                (typeComponents . getType) e ++ acc) []
