...
markStats mark s@Assign {lhs, rhs} = toStat s{lhs= lhs, rhs=toExpr rhs}
markStats mark s@NewWithInit {args} =mark s{args=map toExpr args}
markStats mark s@Print {args} =toStat s{args=map toExpr args}
...
markStats mark s@PartySeq{par, seqfunc} = mark s{par=toExpr par, seqfunc=toExpr seqfunc}
markStats mark s@PartyPar{parl, parr} = mark s{parl=toExpr parl, parr=toExpr parr}
markStats mark s@PartyReduce{pinit, seqfun, par} = mark s{pinit=toExpr pinit, seqfun=toExpr seqfun, par=toExpr par}
