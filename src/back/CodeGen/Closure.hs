...
translateClosure :: A.Expr -> [Type] -> TableLookup -> Ctx.LexicalContext -> CCode Toplevel
translateClosure closure typeVars tlookup scope
    | A.isClosure closure =
       let arrowType   = A.getType closure
           ...
           ctx = Ctx.new subst tlookup scope
