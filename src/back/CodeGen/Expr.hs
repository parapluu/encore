...
type_to_printf_fstr ty
  ...
  | otherwise = case translate ty of
                     Ptr _ -> "%p"
                     _     -> "Expr.hs: type_to_printf_fstr not defined for " ++
                       show ty
...
-- This is exclusively used for A.Embed translation:
...
instance Translatable A.Expr (State Ctx.Context (CCode Lval, CCode Stat)) where
    ...
    translate skip@(A.Skip {}) = named_tmp_var "skip" (A.getType skip)
        (AsExpr unit)
    translate null@(A.Null {}) = named_tmp_var "literal" (A.getType null) Null
    translate true@(A.BTrue {}) = named_tmp_var "literal" (A.getType true)
        (Embed "1/*True*/" :: CCode Expr)
    translate false@(A.BFalse {}) = named_tmp_var "literal" (A.getType false)
        (Embed "0/*False*/" :: CCode Expr)
    translate lit@(A.IntLiteral {A.intLit = i}) = named_tmp_var "literal"
        (A.getType lit) (Int i)
    translate lit@(A.RealLiteral {A.realLit = r}) = named_tmp_var "literal"
        (A.getType lit) (Double r)
    translate lit@(A.StringLiteral {A.stringLit = s}) = named_tmp_var "literal"
        (A.getType lit) (String s)
    ...
    translate (A.Print {A.stringLit = s, A.args}) = do
        ...
        let arg_names = map (AsExpr . fst) targs
            arg_decls = map snd targs
            arg_tys   = map A.getType args
            fstring = format_string s arg_tys
              where
                  format_string s [] = s
                  format_string "" (_ty:_tys) =
                      error "Wrong number of arguments to printf"
                  format_string ('{':'}':s) (ty:tys) =
                          type_to_printf_fstr ty ++ format_string s tys
    ...
    translate (A.Assign {A.lhs, A.rhs}) = do
        (nrhs, trhs) <- if A.isThisAccess rhs &&
                           Ty.isActiveRefType (A.getType rhs)
                        then return (Deref (Var "this") `Dot` Nam "aref", Skip)
                        else translate rhs
        lval <- mk_lval lhs
        return (unit, Seq [trhs, Assign lval nrhs])
          where
              mk_lval (A.VarAccess {A.name}) = do
                  ctx <- get
                  case Ctx.subst_lkp ctx name of
                       Just subst_name -> return subst_name
                       Nothing -> return $ Var (show name)
              mk_lval (A.FieldAccess {A.target, A.name}) = do
                  (ntarg, ttarg) <- translate target
                  return (Deref (StatAsExpr ntarg ttarg)
                            `Dot` (Nam $ show name))
              mk_lval e =
                  error $ "Cannot translate '" ++ show e ++ "' to a valid lval"
    ...
    translate acc@(A.FieldAccess {A.target, A.name}) = do
        ...
        return (Var tmp,
                Seq [ ttarg
                    , Assign (Decl (translate (A.getType acc), Var tmp))
                        (Deref ntarg `Dot` Nam (show name))
                    ])

    translate (A.Let {A.decls, A.body}) = do
        tmps_tdecls <- mapM translate_decl decls
        let (_tmps, tdecls) = unzip tmps_tdecls
        (nbody, tbody) <- translate body
        mapM_ (unsubstitute_var . fst) decls
        return (nbody, Seq $ concat tdecls ++ [tbody])
          where
              translate_decl (name, expr) = do
                  (ne, te) <- if A.isThisAccess expr &&
                                 Ty.isActiveRefType (A.getType expr)
                              then return (Deref (Var "this") `Dot`
                                            Nam "aref", Skip)
                              else translate expr
                  ...
                  return (Var tmp,
                            ...
                            , te
                            , Assign
                                (Decl (translate (A.getType expr), Var tmp)) ne
                            ])

    translate (A.New {A.ty})
      | Ty.isActiveRefType ty =
          named_tmp_var "new" ty (Call (Nam "create_and_send")
              [Amp $ actor_rec_name ty
              , AsExpr . Var $ "MSG_alloc"
              ])
      | otherwise = do
          na <- Ctx.gen_named_sym "new"
          let size = Sizeof $ Typ $ show (data_rec_name ty)
          return (Var na, Seq [ Assign (Decl (translate ty, Var na))
      ...
      where
          sync_call = do
              ...
              return (Var tmp,
                        Seq [ ttarget
                            , Assign
                                (Decl (translate (A.getType call), Var tmp))
          ...
          remote_call = do
              ttarget <- varaccess_this_to_aref target
              targs <- mapM varaccess_this_to_aref args
              let targtys = map (translate . A.getType) args :: [CCode Ty]
              the_fut_name <- if Ty.isStreamType $ A.getType call
                              then Ctx.gen_named_sym "stream"
                              else Ctx.gen_named_sym "fut"
              let the_fut_decl = if Ty.isStreamType $ A.getType call
                                 then Assign
                                    (Decl (Ptr $ Typ "stream_t",
                                            Var the_fut_name))
                                    (Call (Nam "stream_mk")
                                        ([] :: [CCode Lval]))
                                 else Assign
                                    (Decl (Ptr $ Typ "future_t",
                                            Var the_fut_name))
                                    (Call (Nam "future_mk")
                                        ([] :: [CCode Lval]))
              ...
              let the_arg_decl = Assign
                                   (Decl (Typ "pony_arg_t",
                                            ArrAcc (1 + length args)
                                                (Var the_arg_name)))
                                   (Record
                                     (UnionInst (Nam "p") (Var the_fut_name) :
                                     map (\(arg, ty) ->
                                            UnionInst (pony_arg_t_tag ty) arg)
                                         (zip targs targtys) :: [CCode Expr]))
                  the_call = Call (Nam "pony_sendv")
                              [ ttarget
                              , AsExpr . AsLval $
                                    method_msg_name (A.getType target) name
                              , Int $ 1 + length args
    ...
    translate (A.MessageSend {A.target=target, A.name=name, A.args=args})
      ...
      where
          ...
          message_send = do
              ...
              let the_arg_decl = Assign
                      (Decl (Typ "pony_arg_t",
                             ArrAcc (1 + length args) (Var the_arg_name)))
                         (Record (map (\(arg, ty) ->
                                       UnionInst (pony_arg_t_tag ty) arg)
                                   (zip targs targtys) :: [CCode Expr]))
                  the_call = Call (Nam "pony_sendv")
                              [ ttarget
                              , AsExpr . AsLval $
                                  one_way_send_msg_name (A.getType target) name
                              , Int $ length args
                              , AsExpr $ Var the_arg_name
                              ]
              return (unit, Seq (Comm "message send" : the_arg_decl :
                              [Statement the_call]))
    ...
    translate w@(A.While {A.cond, A.body}) = do
        ...
        return (Var tmp, Seq [ Statement $ Decl (translate (A.getType w),
                                                 Var tmp)
                             , While (StatAsExpr ncond tcond)
                                (Statement export_body)
                             ])

    translate ite@(A.IfThenElse {A.cond, A.thn, A.els}) = do
        tmp <- Ctx.gen_named_sym "ite"
        (ncond, tcond) <- translate cond
        ...
        return (Var tmp,
                Seq [ AsExpr $ Decl (translate (A.getType ite), Var tmp)
                    , If (StatAsExpr ncond tcond) (Statement export_thn)
                        (Statement export_els)
                    ])

    translate e@(A.Embed {A.code=code}) = do
        interpolated <- interpolate code
        if Ty.isVoidType (A.getType e)
        then return (unit, Embed $ "({" ++ interpolated ++ "})")
        else named_tmp_var "embed" (A.getType e)
            (Embed $ "({" ++ interpolated ++ "})")
          where
            interpolate :: String -> State Ctx.Context String
            interpolate embedstr =
                case Parsec.parse interpolate_parser "embed expression"
                        embedstr of
                  (Right parsed) -> do
            ...
            to_looked_up_string :: Either String VarLkp
                                -> State Ctx.Context String
            to_looked_up_string e =
                case e of
                     Right (VarLkp var) -> do
                         ctx <- get
            ...
            interpolate_parser = Parsec.many $ varlkp Parsec.<|> chars
              where
                  varlkp = do
                      var <- varlkp_parser
    ...
    translate (A.Get{A.val})
      | Ty.isFutureType $ A.getType val = do
          ...
          tmp <- Ctx.gen_sym
          return (Var tmp,
                  Seq [tval, Assign (Decl (result_type, Var tmp)) the_get])
      | Ty.isStreamType $ A.getType val = do
          (nval, tval) <- translate val
          let result_type = translate (Ty.getResultType $ A.getType val)
              the_get = Cast result_type $ Call (Nam "stream_get") [nval] `Dot`
                            pony_arg_t_tag result_type
          tmp <- Ctx.gen_sym
          return (Var tmp,
                  Seq [tval, Assign (Decl (result_type, Var tmp)) the_get])
    ...
    translate (A.Yield{A.val}) = do
        (nval, tval) <- translate val
        let yield_arg = Cast pony_arg_t $ UnionInst
                (pony_arg_t_tag (translate (A.getType val))) nval
            update_stream = Assign stream_handle (Call (Nam "stream_put")
                                                       [ AsExpr stream_handle
                                                       , yield_arg
                                                       ])
    ...
    translate (A.IsEos{A.target}) = do
        (ntarg, ttarg) <- translate target
        tmp <- Ctx.gen_sym
        let the_call =
                Assign (Decl (bool, Var tmp)) (Call (Nam "stream_eos") [ntarg])
        return (Var tmp, Seq [ttarg, the_call])

    translate (A.StreamNext{A.target}) = do
        (ntarg, ttarg) <- translate target
        tmp <- Ctx.gen_sym
        let the_call = Assign (Decl (stream, Var tmp))
                        (Call (Nam "stream_get_next") [ntarg])
        return (Var tmp, Seq [ttarg, the_call])

    translate (A.Await{A.val}) = do
        (nval, _tval) <- translate val
        tmp <- Ctx.gen_sym
        return (Var tmp, Seq [Call (Nam "future_await") [nval]])

    translate (A.Suspend{}) = do
        tmp <- Ctx.gen_sym
        return (Var tmp, Seq [Call (Nam "future_suspend") [Nam ""]])
        --TODO: Call should support 0-arity

    translate (A.FutureChain{A.future, A.chain}) = do
        (nfuture,tfuture) <- translate future
        (nchain, tchain)  <- translate chain
        fut_name <- Ctx.gen_sym
        let fut_decl = Assign (Decl (Ptr $ Typ "future_t", Var fut_name))
        ...
        return (Var result,
                Seq [ tfuture
                    , tchain
                    , fut_decl
                    , Assign (Decl (Ptr $ Typ "future_t", Var result))
                        (Call (Nam "future_chain_actor")
                        [nfuture, Var fut_name, nchain])
                    ])

    translate clos@(A.Closure{A.eparams, A.body}) = do
        let fun_name = closure_fun_name . Meta.getMetaId . A.getMeta $ clos
              ...
              mk_env name =
                  Assign (Decl (Ptr $ Struct name, AsLval name))
                          (Call (Nam "pony_alloc") -- Use malloc until we figure out how to trace environments
                                [Sizeof $ Struct name])
              insert_var env_name (name, _) = do
                  c <- get
                  let tname = fromMaybe (Var $ show name) (Ctx.subst_lkp c name)
                  return $ Assign ((Deref $ Var $ show env_name) `Dot`
                                    (Nam $ show name)) tname
    ...
    translate fcall@(A.FunctionCall{A.name, A.args}) = do
        ...
        (calln, the_call) <- named_tmp_var "clos" ty $ arg_member ty
                                (Call (Nam "closure_call") [clos, tmp_args])
        let comment = Comm $ "fcall name: " ++ show name ++
                             " (" ++ show (Ctx.subst_lkp c name) ++ ")"
            value = if Ty.isVoidType ty then unit else calln
        return (value, Seq [comment, tmp_arg_decl, the_call])
          where
              ...
              translateArgument arg = do
                  (ntother, tother) <- if A.isThisAccess arg &&
                                          Ty.isActiveRefType (A.getType arg)
                                       then return (Deref (Var "this") `Dot`
                                                    Nam "aref", Skip)
                                       else translate arg
                  return $ UnionInst (arg_member $ A.getType arg)
