...
instance Translatable A.ClassDecl (NamespaceTable -> CCode FIN) where
  translate cdecl ntable
      | A.isActive cdecl = translateActiveClass cdecl ntable
      | A.isShared cdecl = translateSharedClass cdecl ntable
      | otherwise        = translatePassiveClass cdecl ntable

-- | Translates an active class into its C representation. Note
-- that there are additional declarations in the file generated by
-- "CodeGen.Header"
translateActiveClass cdecl@(A.Class{A.cname, A.cfields, A.cmethods}) ntable@(ftable, ctable) =
    Program $ Concat $
...
translateSharedClass cdecl@(A.Class{A.cname, A.cfields, A.cmethods}) ntable@(ftable, ctable) =
  Program $ Concat $
    (LocalInclude "header.h") :
    [traitMethodSelector ctable cdecl] ++
    [typeStructDecl cdecl] ++
    ...
    where
      methodImpls = map methodDecl cmethods
        where
          methodDecl mdecl = translate mdecl cdecl ntable
...
-- "CodeGen.Header"
translatePassiveClass cdecl@(A.Class{A.cname, A.cfields, A.cmethods}) ntable@(ftable, ctable) =
  Program $ Concat $
    (LocalInclude "header.h") :
    [traitMethodSelector ctable cdecl] ++
    [runtimeTypeInitFunDecl cdecl] ++
