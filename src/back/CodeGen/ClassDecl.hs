...
instance Translatable A.ClassDecl (ClassTable -> FunctionTable -> CCode FIN) where
  translate cdecl ctable ftable
      | A.isActive cdecl = translateActiveClass cdecl ctable ftable
      | A.isShared cdecl = translateSharedClass cdecl ctable ftable
      | otherwise        = translatePassiveClass cdecl ctable ftable

-- | Translates an active class into its C representation. Note
-- that there are additional declarations in the file generated by
-- "CodeGen.Header"
translateActiveClass cdecl@(A.Class{A.cname, A.cfields, A.cmethods}) ctable ftable =
    Program $ Concat $
    ...
    where
      methodImpls = map methodImpl cmethods
        where
          methodImpl mdecl = translate mdecl cdecl ctable ftable
...
translateSharedClass cdecl@(A.Class{A.cname, A.cfields, A.cmethods}) ctable ftable =
  Program $ Concat $
    (LocalInclude "header.h") :
    [traitMethodSelector ctable cdecl] ++
    [typeStructDecl cdecl] ++
    ...
    where
      methodImpls = map methodDecl cmethods
        where
          methodDecl mdecl = translate mdecl cdecl ctable ftable
...
-- "CodeGen.Header"
translatePassiveClass cdecl@(A.Class{A.cname, A.cfields, A.cmethods}) ctable ftable =
  Program $ Concat $
    (LocalInclude "header.h") :
    [traitMethodSelector ctable cdecl] ++
    [runtimeTypeInitFunDecl cdecl] ++
