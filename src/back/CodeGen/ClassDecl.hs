...
instance Translatable A.ClassDecl (TableLookup -> CCode FIN) where
  translate cdecl tableLookup
      | A.isActive cdecl = translateActiveClass cdecl tableLookup
      | A.isShared cdecl = translateSharedClass cdecl tableLookup
      | otherwise        = translatePassiveClass cdecl tableLookup

-- | Translates an active class into its C representation. Note
-- that there are additional declarations in the file generated by
-- "CodeGen.Header"
translateActiveClass cdecl@(A.Class{A.cname, A.cfields, A.cmethods}) tableLookup@(CT ct) =
    Program $ Concat $
      (LocalInclude "header.h") :
      [traitMethodSelector ct cdecl] ++
    ...
    where
      ...
      methodImpl mdecl = translate mdecl cdecl tableLookup
...
translateSharedClass cdecl@(A.Class{A.cname, A.cfields, A.cmethods}) tableLookup@(CT ct) =
  Program $ Concat $
    (LocalInclude "header.h") :
    [traitMethodSelector ct cdecl] ++
    [typeStructDecl cdecl] ++
    ...
    where
      methodImpls = map methodDecl cmethods
        where
          methodDecl mdecl = translate mdecl cdecl tableLookup
...
-- "CodeGen.Header"
translatePassiveClass cdecl@(A.Class{A.cname, A.cfields, A.cmethods}) tableLookup@(CT ct) =
  Program $ Concat $
    (LocalInclude "header.h") :
    [traitMethodSelector ct cdecl] ++
    [runtimeTypeInitFunDecl cdecl] ++
...
runtimeTypeInitFunctionDecl :: A.Function -> CCode Toplevel
runtimeTypeInitFunctionDecl f@(A.Function {}) =
  let name = A.functionName f
      ty = A.functionType f
  in
    Function void (runtimeTypeInitFunctionName name)
                 [(Ptr . AsType $ functionTypeName name, Var "this"), (Embed "...", Embed "")]
                   (Seq $
                    (Statement $ Decl (Typ "va_list", Var "params")) :
                    (Statement $ Call (Nam "va_start") [Var "params", Var "this"]) :
                    map initRuntimeType typeParams ++
