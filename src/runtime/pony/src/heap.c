...
{
  HEAP_MIN, 128, 256, 512, 1024, HEAP_MAX
};
...
{
  if(chunk->size <= (1 << (POOL_SIZE_BITS - 1)))
    POOL_FREESIZE(chunk->size, chunk->m);
  else
    munmap(chunk->m, chunk->size);
  ...
  while(chunk != NULL)
  {
    ...
    if(chunk->slots == 0)
    {
      ...
      *full = chunk;
      ...
      used += sizeof(block_t) -
        (__builtin_popcount(chunk->slots) * sizeclass_size[chunk->size]);
      chunk->next = *avail;
      *avail = chunk;
    }
...
static void* large_malloc(pony_actor_t* actor, heap_t* heap, size_t size)
{
  ...
  if(size <= (1 << (POOL_SIZE_BITS - 1)))
  {
    chunk->size = next_pow2(size);
    chunk->m = POOL_ALLOCSIZE(chunk->size);
  } else {
    chunk->size = size & ~HEAP_PAGEMASK;
    if((size & HEAP_PAGEMASK) != 0) { chunk->size += HEAP_PAGESIZE; }
