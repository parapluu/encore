...
checkType ty
  ...
  | isTypeVar ty = do
      ...
      unless (ty `elem` params) $ tcError $ "Free type variables in type '" ++
        show ty ++ "'"
      return ty
  | isRefType ty = do
      result <- asks $ classTypeLookup ty
      params <- mapM checkType (getTypeParameters ty)
      case result of
           Nothing -> tcError $ "Unknown type '" ++ show ty ++ "'"
           Just refType -> do
               let formalParams = getTypeParameters refType
               unless (length params == length formalParams) $
                   tcError $ "Class '" ++ show refType ++ "' " ++
                     "expects " ++ show (length formalParams) ++
                       " type parameters.\n" ++ "Type '" ++ show ty ++
                         "' has " ++ show (length params)
               if isActiveRefType refType
               then return $ makeActive $ setTypeParameters ty params
               else return $ makePassive $ setTypeParameters ty params
  | isFutureType ty = do
      ty' <- checkType $ getResultType ty
  ...
  | isParType ty = do
      ...
      return $ parType ty'
  | isArrowType ty = do
      argTypes <- mapM checkType (getArgTypes ty)
...
class Checkable a where
    ...
    pushHasType :: Pushable a => a -> Type
                -> ExceptT TCError (Reader Environment) a
...
instance Checkable Program where
    ...
    -- ----------------------------
    --  E |- funs classes
    typecheck (Program etl imps funs classes) = do
...
instance Checkable Function where
   ...
   ---  |- t1 .. |- tn
    ...
    typecheck f@(Function {funtype, funparams, funbody}) = do
        let typeParams = nub $ filter isTypeVar $ concatMap
                (typeComponents . ptype) funparams
        ty <- local (addTypeParameters typeParams) $ checkType funtype
        eParams <- mapM (local (addTypeParameters typeParams) . typecheckParam)
                    funparams
        eBody <- local (addParams eParams) $ pushHasType funbody ty
        return $ setType ty f {funtype = ty, funbody = eBody,
                               funparams = eParams}
      where
          typecheckParam p@(Param{ptype}) = local (pushBT p) $ do
              ty <- checkType ptype
              return $ setType ty p
          addParams params = extendEnvironment $ map (\(Param {pname, ptype}) ->
                                                        (pname, ptype)) params
...
instance Checkable ClassDecl where
   ...
   ---  |- field1 .. |- fieldn
    ...
    typecheck c@(Class {cname, fields, methods}) = do
        distinctTypeParams
        distinctFieldNames
        efields <- mapM (withParams . pushTypecheck) fields
        distinctMethodNames
      ...
      where
          ...
          typecheckMethod m = withParams $ local
              (extendEnvironment [(thisName, cname)]) $ pushTypecheck m
          distinctTypeParams =
              ...
              in
                case params \\ paramsNoDuplicates of
                     [] -> return ()
                     (p:_) -> tcError $ "Duplicate type parameter '" ++
                       show p ++ "'"
          distinctFieldNames =
              let fieldsNoDuplicates = nubBy ((==) `on` fname) fields
              in
                case fields \\ fieldsNoDuplicates of
                     ...
                     (f:_) -> do
                         bt <- asks backtrace
                         throwError $
                           TCError ("Duplicate definition of field '" ++
                             show (fname f) ++ "'" , push f bt)
          distinctMethodNames =
              case methods \\ nubBy ((==) `on` mname) methods of
                   [] -> return ()
                   (m:_) -> do
                       bt <- asks backtrace
                       throwError $
                         TCError ("Duplicate definition of method '" ++
                             show (mname m) ++ "'" , push m bt)
...
instance Checkable MethodDecl where
   ...
   ---  |- t1 .. |- tn
    ...
    typecheck m@(Method {mtype, mparams, mbody, mname}) = do
        let typeParams = nub $ filter isTypeVar $
                  concatMap (typeComponents . ptype) mparams
        ...
        eMparams <- mapM (local (addTypeParameters typeParams) . typecheckParam)
                        mparams
        eBody <- local (addParams eMparams) $ pushHasType mbody ty
        return $ setType ty m {mtype = ty, mbody = eBody, mparams = eMparams}
      where
          checkMainParams = unless (map ptype mparams `elem`
                                   [[] {-, [intType, arrayType stringType]-}]) $
                              tcError $ "Main method must have argument " ++
                                "type () or (int, string[]) " ++
                                  "(but arrays are not supported yet)"
          typecheckParam p@(Param{ptype}) = local (pushBT p) $ do
              ty <- checkType ptype
              return $ setType ty p
          addParams params = extendEnvironment $ map
            (\(Param {pname, ptype}) -> (pname, ptype)) params
   ...
   ---  |- t1 .. |- tn
    ...
    typecheck m@(StreamMethod {mtype, mparams, mbody}) = do
        Just thisType <- asks $ varLookup thisName
...
instance Checkable Expr where
    hasType expr ty = do
        ...
        if isNullType exprType
        then coerceNull eExpr ty
        else do
            assertSubtypeOf exprType ty
            return eExpr
      where
          coerceNull null ty
            | isNullType ty || isTypeVar ty =
   ...
   ---  |- t
    ...
    -- ----------------------
    --  E |- (body : t) : t
    typecheck (TypedExpr {body, ty}) = do
        ty' <- checkType ty
        pushHasType body ty'
    ...
    typecheck mcall@(MethodCall {target, name, args}) = do
        ...
        unless (isRefType targetType) $
          tcError $ "Cannot call method on expression '" ++
            show (ppExpr target) ++ "' of type '" ++ show targetType ++ "'"
        when (isMainType targetType && name == Name "main")
            (tcError "Cannot call the main method")
        when (name == Name "init")
            (tcError $ "Constructor method 'init' can only be called " ++
        ...
        unless (length args == length paramTypes) $
          tcError $
            (case name of
                ...
                _ -> "Method '" ++ show name ++ "'") ++
               " of class '" ++ show targetType ++ "' expects " ++
                 show (length paramTypes) ++ " arguments. Got " ++
                   show (length args)
        ...
        (eArgs, bindings) <- local (bindTypes targetBindings) $ matchArguments
            args paramTypes
        let resultType = replaceTypeVars bindings methodType
            returnType
              | isThisAccess target || isPassiveRefType targetType = resultType
              | isStreamMethod mdecl = streamType resultType
              | otherwise = futureType resultType
    ...
    typecheck msend@(MessageSend {target, name, args}) = do
        ...
        formalTypeParams <- asks $ classTypeParameterLookup targetType
        targetBindings <- matchTypeParameters formalTypeParams actualTypeParams
        (eArgs, _bindings) <- local (bindTypes targetBindings) $ matchArguments
            args paramTypes
    ...
    typecheck fcall@(FunctionCall {name, args}) = do
        ...
        ty <- case funType of
                   Just ty -> return ty
                   Nothing -> tcError $ "Unbound function variable '" ++
                     show name ++ "'"
   ...
   ---  |- t1 .. |- tn
    ...
    typecheck closure@(Closure {eparams, body}) = do
        let typeParams = nub $ filter isTypeVar $
                            concatMap (typeComponents . ptype) eparams
        eEparams <- mapM (local (addTypeParameters typeParams) . typecheckParam)
                        eparams
        ...
        return $ setType (arrowType (map ptype eparams) returnType)
            closure {body = eBody, eparams = eEparams}
      where
          typecheckParam p@(Param{ptype}) = local (pushBT p) $ do
              ty <- checkType ptype
              return $ setType ty p
    ...
    -- ------------------------
    --  E |- {e1; ..; en} : t
    typecheck seq@(Seq {eseq}) = do
        eEseq <- mapM pushTypecheck eseq
    ...
    typecheck ifThenElse@(IfThenElse {cond, thn, els}) = do
        eCond <- pushHasType cond boolType
      ...
      where
          matchBranches ty1 ty2
            ...
            | otherwise =
                if ty2 == ty1
                then return ty1
                else tcError $ "Type mismatch in different branches " ++
                               "of if-statement:\n" ++
                               "  then:  " ++ show ty1 ++ "\n" ++
                               "  else:  " ++ show ty2
    ...
    -- -----------------------
    --  E |- while cond body : t
    typecheck while@(While {cond, body}) = do
        eCond <- pushHasType cond boolType
    ...
    -- ------------------
    --  E |- get val : t
    typecheck get@(Get {val}) = do
        eVal <- pushTypecheck val
        let ty = AST.getType eVal
        unless (isFutureType ty || isStreamType ty) $
          tcError $ "Cannot get the value of non-future type '" ++
            show ty ++ "'"
    ...
    -- ----------------------------
    --  E |- eos : void
    typecheck eos@(Eos {}) = do
        bt <- asks backtrace
        let mtd = currentMethod bt
        unless (isStreamMethod mtd) $
          tcError $ "Cannot have end-of-stream in non-streaming " ++
            "method '" ++ show (mname mtd) ++ "'"
        return $ setType voidType eos

    --  E |- s : Stream t
    -- ---------------------
    --  E |- eos s : bool
    typecheck iseos@(IsEos {target}) = do
        eTarget <- pushTypecheck target
    ...
    -- ---------------------------
    --  E |- s.next() : Stream t
    typecheck next@(StreamNext {target}) = do
        eTarget <- pushTypecheck target
    ...
    --    ------------------ :: await
    --    await f : void
    typecheck await@(Await {val}) = do
        eVal <- pushTypecheck val
        let ty = AST.getType eVal
        unless (isFutureType ty) $
          tcError $ "Cannot await the value of non-future type '" ++
            show ty ++ "'"
    ...
    --    ------------------ :: chain
    --    f then c : Fut T'
    typecheck futureChain@(FutureChain {future, chain}) = do
        eFuture <- pushTypecheck future
        ...
        unless ([getResultType ty] == getArgTypes ty') $
          tcError $ "Future value has type '" ++ show (getResultType ty) ++
            "' but chained closure expects '" ++
              show (head (getArgTypes ty')) ++ "'"
        return $ setType (futureType (getResultType ty'))
    ...
    typecheck fAcc@(FieldAccess {target, name}) = do
        ...
        when (isPrimitive targetType) $
          tcError $ "Cannot read field of expression '" ++
               show (ppExpr target) ++ "' of primitive type '" ++
                 show targetType ++ "'"
        when (isTypeVar targetType) $
          tcError $ "Cannot read field of expression '" ++
            show (ppExpr target) ++ "' of polymorphic type '" ++
              show targetType ++ "'"
        when (isActiveRefType targetType && not (isThisAccess eTarget)) $
          tcError $ "Cannot read field of expression '" ++
            show (ppExpr target) ++ "' of active object type '" ++
              show targetType ++ "'"
        ...
        case fdecl of
             Just Field{ftype} -> do
                let actualTypeParams = getTypeParameters targetType
                formalTypeParams <- asks $ classTypeParameterLookup targetType
                bindings <- matchTypeParameters formalTypeParams
                                actualTypeParams
                let ty' = replaceTypeVars bindings ftype
                return $ setType ty' fAcc {target = eTarget}
             Nothing -> tcError $ "No field '" ++ show name ++ "' in class '" ++
               show targetType ++ "'"
    ...
    -- ------------------------
    --  E |- name = rhs : void
    typecheck assign@(Assign {lhs = lhs@VarAccess{name}, rhs}) = do
        eLhs <- pushTypecheck lhs
    ...
    typecheck assign@(Assign {lhs, rhs}) = do
        eLhs <- pushTypecheck lhs
        unless (isLval lhs) $
          tcError $ "Left hand side '" ++ show (ppExpr lhs) ++
    ...
    -- ----------------
    --  E |- name : t
    typecheck var@(VarAccess {name}) = do
   ...
   ---  |- t
    ...
    typecheck print@(Print {stringLit, args}) = do
        let noArgs = T.count (T.pack "{}") (T.pack stringLit)
    ...
    typecheck stringLit@(StringLiteral {}) =
    ...
    typecheck binop@(Binop {op, loper, roper})
      | op `elem` boolOps = do
          ...
          unless (isBoolType lType && isBoolType rType) $
            tcError $ "Operator '"++ show op ++
              "' is only defined for boolean types"
      ...
      | op `elem` cmpOps = do
          ...
          unless (isNumeric lType && isNumeric rType) $
            tcError $ "Operator "++ show op ++
              " is only defined for numeric types"
          return $ setType boolType binop {loper = eLoper, roper = eRoper}
      | op `elem` eqOps = do
          eLoper <- pushTypecheck loper
      ...
      | op `elem` arithOps = do
          ...
          unless (isNumeric lType && isNumeric rType) $
            tcError $ "Operator "++ show op ++
              " is only defined for numeric types"
          return $ setType (coerceTypes lType rType)
              binop {loper = eLoper, roper = eRoper}
      ...
      where
          ...
          cmpOps   = [Identifiers.LT, Identifiers.GT,
                      Identifiers.LTE, Identifiers.GTE]
...
matchArguments :: [Expr] -> [Type]
               -> ExceptT TCError (Reader Environment) ([Expr], [(Type, Type)])
matchArguments [] [] = do
    bindings <- asks bindings
    return ([], bindings)
matchArguments (arg:args) (typ:types) = do
    eArg <- pushTypecheck arg
  ...
  | isFutureType expected && isFutureType ty ||
    ...
    isStreamType expected && isStreamType ty =
        ...
        `catchError` (\_ -> tcError $ "Type '" ++ show ty ++
          "' does not match expected type '" ++ show expected ++ "'")
