...
instance Show BacktraceNode where
    show (BTFunction n ty) = "In function '" ++ show n ++ "' of type '" ++
      show ty ++ "'"
    show (BTClass ty) = "In class '" ++ show ty ++ "'"
    show (BTParam p) = "In parameter '" ++ show (ppParamDecl p) ++ "'"
    show (BTField f) = "In field '" ++ show (ppFieldDecl f) ++ "'"
    show (BTMethod Method{mname, mtype}) = "In method '" ++ show mname ++
      "' of type '" ++ show mtype ++ "'"
    show (BTMethod StreamMethod{mname, mtype}) = "In stream method '" ++
      show mname ++ "' of type '" ++ show mtype ++ "'"
    show (BTExpr expr)
...
currentMethod [] = error $ "*** Internal error ***\nTried to get current" ++
                           "method when not in a method"
...
instance Pushable Function where
    push fun@(Function {funname, funtype}) bt =
        (getPos fun, BTFunction funname funtype) : bt
