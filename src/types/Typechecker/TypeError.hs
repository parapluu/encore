...
data Error =
  ...
  | TypeVariableAndVariableCommonName [Name]
...
instance Show Error where
    ...
    show (TypeVariableAndVariableCommonName names) =
        printf ("Type variable(s) %s clash with existing variable name.\n" ++
                "Rename the (type) variable(s)")
                formattingName
        where
          formattingName =
            let ns = map (\n -> "'" ++ show n ++ "', ") (init names)
                lastName = "'" ++ (show $ last names) ++ "'"
            in if null ns then lastName
               else (show ns) ++ "and " ++ lastName
